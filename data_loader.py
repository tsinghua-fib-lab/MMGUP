...
tune_user_set = defaultdict(list)
...
def read_cf(file_name):
    ...
    for l in lines:
        user, poi = l.strip('\n').split('\t')
        user, poi = int(user), int(poi)
        if user not in user_inter_dict:
            user_inter_dict[user] = []
        user_inter_dict[user].append(poi)
    user_num = len(user_inter_dict)
    for user in range(user_num):
        u_id, p_ids = user, list(set(user_inter_dict[user]))
        for i_id in p_ids:
            inter_mat.append([u_id, i_id])
    return np.array(inter_mat)


def remap_item(train_data, tune_data, test_data):
    global n_users, n_items
    n_users = max(max(train_data[:, 0]), max(tune_data[:, 0]), max(test_data[:, 0])) + 1
    n_items = max(max(train_data[:, 1]), max(tune_data[:, 1]), max(test_data[:, 1])) + 1

    for u_id, i_id in train_data:
        train_user_set[int(u_id)].append(int(i_id))
    for u_id, i_id in tune_data:
        tune_user_set[int(u_id)].append(int(i_id))
...
def read_triplets(file_name):
    ...
    print(n_nodes, n_entities, n_users)
...
def load_data(model_args):
    ...
    print('target: KGIN on kuaishou')
    print('reading train and test user-item set ...')
    train_cf = read_cf(directory + 'train_mini.txt')
    tune_cf = read_cf(directory + 'tune_mini.txt')
    test_cf = read_cf(directory + 'test_mini.txt')
    remap_item(train_cf, tune_cf, test_cf)

    print('combinating train_cf and kg data ...')
    triplets = read_triplets(directory + 'total_final_kg.txt')
    ...
    user_dict = {
        ...
        'tune_user_set': tune_user_set,
        'test_user_set': test_user_set
    }

    return train_cf, tune_cf, test_cf, user_dict, n_params, graph, \
